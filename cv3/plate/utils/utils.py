...
def apply_classifier(x, model, img, im0):
    ...
    for i, d in enumerate(x):  # per image
        if d is not None and len(d):
            ...
            # plat_num=np.zeros((np.array(preb_labels).shape[0],7))

            # for i in range(plat_num.shape[0]):
            #     plat_num[i,:]=plat_lb[i,:]
            # plat_num = torch.FloatTensor(plat_num)
            # x[i]=torch.cat([x[i],plat_num],1)

            # x[i] = x[i][pred_cls1 == preb_labels]  # retain matching class detections
